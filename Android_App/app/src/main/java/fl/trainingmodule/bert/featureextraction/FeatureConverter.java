...
import fl.trainingmodule.bert.tokenization.FullTokenizer;

/** Convert String to features that can be fed into BERT model. */
public final class FeatureConverter {
  private static int maxQueryLen = 64;
  private static int maxSeqLen = 384;


  public static Feature convert(Map<String, Integer> inputDic, boolean doLowerCase, String query, String context) {

    FullTokenizer tokenizer = new FullTokenizer(inputDic, doLowerCase);
    ...
    for (int i = 0; i < origTokens.size(); i++) {
      ...
      List<String> subTokens = FullTokenizer.tokenize(token);
    ...
    List<Integer> inputIds = FullTokenizer.convertTokensToIds(tokens);
